...
export default function Hero() {
  ...
  const MY_PHONE_NUMBER = "01701664898";

  const handleCall = () => {
    const isMobile = /Android|iPhone|iPad/i.test(navigator.userAgent);

    if (isMobile) {
      window.location.href = `https://wa.me/call/${MY_PHONE_NUMBER}`;
    } else {
      // Desktop fallback
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="space-y-8 animate-fade-in mt-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              onClick={handleCall}
              className="group px-8 py-2 bg-[#FF6600] text-white rounded-full font-bold flex items-center justify-center space-x-2 cursor-pointer"
            >
              <span>Contact For Your Services</span>
              <img src={whatsapp} alt="" srcset="" className="h-10 ml-1" />
            </button>
